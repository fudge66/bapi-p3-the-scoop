...
const database = readYaml.sync('database.yml');
...
function updateComment(url, request) {
  ...
  if (requestComment && requestComment.body && database.comments[commentId]) {
    database.comments[commentId].body = requestComment.body;

    response.status = 200;
    response.body = database.comments[commentId];
  } else if (!database.comments[commentId]) {
    response.status = 404;
  } else {
    response.status = 400;
  }
